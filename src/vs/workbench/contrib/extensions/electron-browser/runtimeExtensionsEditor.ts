		...
		const renderer: IListRenderer<IRuntimeExtension, IRuntimeExtensionTemplateData> = {
			...
			renderElement: (element: IRuntimeExtension, index: number, data: IRuntimeExtensionTemplateData): void => {
				...
				if (this._extensionHostProfileService.getUnresponsiveProfile(element.description.identifier)) {
					const el = $('span');
					el.innerHTML = renderCodicon(` $(alert) Unresponsive`);
				...
				if (isNonEmptyArray(element.status.runtimeErrors)) {
					const el = $('span');
					el.innerHTML = renderCodicon(`$(bug) ${nls.localize('errors', "{0} uncaught errors", element.status.runtimeErrors.length)}`);
					data.msgContainer.appendChild(el);
				}

				if (element.status.messages && element.status.messages.length > 0) {
					const el = $('span');
					el.innerHTML = renderCodicon(`$(alert) ${element.status.messages[0].message}`);
					data.msgContainer.appendChild(el);
				}

				if (element.description.extensionLocation.scheme !== 'file') {
					const el = $('span');
					el.innerHTML = renderCodicon(`$(remote) ${element.description.extensionLocation.authority}`);
					data.msgContainer.appendChild(el);
					...
					if (hostLabel) {
						el.innerHTML = renderCodicon(`$(remote) ${hostLabel}`);
					}
				}
